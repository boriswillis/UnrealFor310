...
#define LOCTEXT_NAMESPACE "HandEngineLiveLink_LLSourceEditor"

void SHandEngineLiveLink_LLSourceFactory::Construct(const FArguments& Args)
{
	...
	+ SHorizontalBox::Slot()
		...
		.OnTextCommitted(this, &SHandEngineLiveLink_LLSourceFactory::OnEndpointAddressChanged)
		]
		]
	...
	+ SHorizontalBox::Slot()
		...
		.OnTextCommitted(this, &SHandEngineLiveLink_LLSourceFactory::OnEndpointPortChanged)
		]*/
		]
	...
	+ SVerticalBox::Slot()
		...
		[
			//Add button
			SNew(SButton)
			.OnClicked(this, &SHandEngineLiveLink_LLSourceFactory::OnOkClicked)
		[
			SNew(STextBlock)
			.Text(LOCTEXT("Ok", "Ok"))
		]
...
void SHandEngineLiveLink_LLSourceFactory::OnEndpointAddressChanged(const FText& NewValue, ETextCommit::Type)
{
	FNumberFormattingOptions form = FNumberFormattingOptions();
	form.SetUseGrouping(false);
...
void SHandEngineLiveLink_LLSourceFactory::OnEndpointPortChanged(const FText& NewValue, ETextCommit::Type)
{
	...
	if (EditabledTextPin_Port.IsValid())
	{
		...
		if (!FIPv4Endpoint::Parse(EditabledTextPin_Address->GetText().ToString() + ":" + NewValue.ToString(), Endpoint))
		{
			...
			Endpoint.Port = 60062;	//Default port number
...
FReply SHandEngineLiveLink_LLSourceFactory::OnOkClicked()
{
	...
	if (EditabledTextPin_Address.IsValid())
	{
		...
		if (FIPv4Endpoint::Parse(EditabledTextPin_Address->GetText().ToString() + ":" + "60062", Endpoint))
		{
